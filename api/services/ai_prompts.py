...
- fair_price.from/to — диапазон для ЭТОГО товара в ЕГО состоянии.
  reasoning: сравни с конкретными аналогами из АЛЬТЕРНАТИВ (номер аналога, цена, отличие).
...
- resale_potential: fast/market/optimal — внутреннее поле с ценовыми ориентирами
  рынка в BYN. В пользовательском тексте называй это только ценовыми ориентирами.
  fast — нижний ориентир рынка; market — справедливая; optimal — верхняя точка
  при хорошем состоянии. reasoning: почему такие ориентиры, с учётом состояния
  и спроса.
- market_context — 2-3 предложения: позиция цены, лучший аналог, тренд.
- best_pick — оставь ad_id=null; reason заполни обычным текстом без id.
- В пользовательских строках НЕ пиши внутренние id объявлений: не используй
  `ad_id`, длинные номера объявлений и формулировки вроде «Объявление 106...».
  Пиши «аналог за 1650 BYN», «вариант от того же продавца», «похожее объявление».
...
_PRICE_MARKET_SCHEMA = """{
  "fair_price": {
    "from": число_BYN,
    "to": число_BYN,
    "reasoning": "обоснование: аналог №1 стоит X BYN в состоянии Y, этот — потому что..."
  },
  "resale_potential": {
    ...
    "reasoning": "общее обоснование: какие ценовые ориентиры реалистичны и почему"
  },
...
- recommendation.verdict — строго: worth_it / think_twice / overpriced.
  Калибровка:
  * worth_it — цена около/ниже медианы или Q1, риски обычные и есть понятный
    план проверки. Не обещай «бери сразу» — объясни, что подтвердить.
  * think_twice — только когда есть конкретная причина осторожности: мало данных,
    цена договорная, спорное состояние, риск-сигналы, слабые фото, подозрительные
    условия. НЕ выбирай think_twice просто из общей осторожности.
  * overpriced — цена выше Q3/медианы без сильного обоснования или товар хуже
...
- АДАПТАЦИЯ К ЛЮБОЙ КАТЕГОРИИ: сначала определи, что именно продают
  (электроника, авто, растение, книга, одежда, мебель, детский товар,
  инструмент, недвижимость, животные, запчасть, услуга/аксессуар). Затем
  подбирай лексику, selling_points, photo_tips, торг и риски именно под эту
  категорию. Если товар не электроника или авто — не используй электронику/авто-шаблоны
  вроде «экран», «разъём», «IMEI», «пробег», «VIN», «двигатель», если они
  явно не относятся к товару.
- Если данных мало, не заполняй пробелы догадками. В selling_points или
  photo_tips добавляй короткое «Уточни: ...» с конкретным недостающим фактом
  для этой категории.
